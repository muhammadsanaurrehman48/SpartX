...
export const Footer = () => {
  ...
  return (
    <footer className="bg-blue-900 text-white relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-blue-900 to-gray-800 opacity-80"></div>
      
      {/* Automotive-themed animated elements */}
      <svg className="absolute inset-0 w-full h-full opacity-10 pointer-events-none" viewBox="0 0 1000 200">
        <g>
          <Wrench
            className="animate-[wrenchSpin_4s_linear_infinite]"
            style={{ transformOrigin: 'center', x: 100, y: 100 }}
          />
          <Wrench
            className="animate-[wrenchSpin_4s_linear_infinite_reverse]"
            style={{ transformOrigin: 'center', x: 900, y: 100 }}
          />
        </g>
      </svg>
      <style>
            ...
            <div className="flex space-x-4">
              {socialIcons.map(icon => (
                ...
                >
                  ...
                  <svg
                    className={`h-6 w-6 transform ${hoveredIcons[icon.name] ? 'scale-125 rotate-12' : 'scale-100'} transition-all duration-300`}
                    fill="currentColor"
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                  >
